...
const QUE_HACER_TYPES = [
  'SportsActivityLocation', 'TouristTrip', 'ExerciseAction',
  'FoodEstablishment', 'WineryTour', 'BoatTrip', 'BusTrip',
];

export async function generateMetadata({ params }: { params: { lang: Locale } }): Promise<Metadata> {
  const dict = await getDictionary(params.lang);
  return { title: dict.what_to_do };
}
...
export default async function QueHacerPage({
  ...
  const filteredTypologies = typologies.filter(
    (t) => QUE_HACER_TYPES.includes(t.schemaOrgType) || QUE_HACER_TYPES.includes(t.typeCode),
  );

  return (
    <>
      <div className="section__header" style={{ marginBottom: '1.5rem' }}>
        <h1 style={{ fontSize: '2rem', color: 'var(--color-primary)' }}>{dict.what_to_do}</h1>
      </div>
      <ResourceList
        lang={lang}
        dict={dict}
